...
// 덧셈
char* addBigNum(const char* num1, const char* num2) {
    int len1 = strlen(num1);
    int len2 = strlen(num2);
    int maxlen = (len1 > len2) ? len1 : len2;
    int carry = 0;

    // 결과 배열의 크기는 두 입력 문자열 중 가장 긴 길이 + 2 (1은 '\0', 1은 자리 올림을 위한 공간)
    ...
    result[0] = carry + '0'; // 첫 번째 자리의 자리 올림
...
//  뺄셈
char* subtractBigNum(const char* num1, const char* num2) {
    int len1 = strlen(num1);
    int len2 = strlen(num2);

    // 결과 배열의 크기는 두 입력 문자열 중 긴 문자열의 길이 + 1 (1은 '\0')
...
// 나눗셈
char* divideBigNum(const char* dividend, const char* divisor, char** remainder) {
...
int main() 
{
    ...
    char* sum = addBigNum(num1, num2);
    char* diff = subtractBigNum(num1, num2);
    char* mul = multiply(num1, num2);
    char* quotient = divideBigNum(num1, num2, NULL);
